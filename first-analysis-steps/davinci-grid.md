...
When you create a job using `prepareGaudiExec('DaVinci','v46r4', myPath='.')`
you get the following message:
```
INFO     Set up App Env at: ./DaVinciDev_v46r4
```
`ganga` has created a folder with a local copy of the DaVinci v46r4 release.
...
equivalent of `lb-run DaVinci/v46r4`, prepare your job and then
...
The list of platforms available for a certain DaVinci version (let's try the DaVinci version we are using `v46r4`), can be viewed by
```bash
$ lb-run --list-platforms DaVinci v46r4
```
The default compiler platform for GaudiExec applications is `x86_64-slc6-gcc62-opt`.
So for some DaVinci versions, including the latest `DaVinci v46r4` a few additional actions are needed to set up your ganga job properly.
